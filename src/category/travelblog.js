...
class TravelBlog extends Component{
    ...
    componentDidMount=()=>{
        ...
        .then((data)=>{
            ...
            var result=data.filter(obj=>obj.category=="travel");
            this.setState({travelBooks:result})
        })
        .catch((error)=>{
            console.log(error);
        })
    ...
    render(){
        const {books,travelBooks}=this.state;
...
<div class="row">
  <div class="leftcolumn">
    <h1 style={{textAlign:'center',fontWeight:'bolder',textShadow:'2px 2px 0px'}}>Travel Blogs</h1>
  {travelBooks.map(
                    book=>
